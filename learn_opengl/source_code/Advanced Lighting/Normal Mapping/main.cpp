...
int main(int argc, char *argv[])
{
    ...
    // load textures
    // -------------
    unsigned int diffuseMap = loadTexture(FileSystem::getPath("brickwall.jpg").c_str());
    unsigned int normalMap  = loadTexture(FileSystem::getPath("brickwall_normal.jpg").c_str());
    ...
    while (main_loop)
    {
        ...
        model = glm::rotate(model, glm::radians((float)(timer+=0.1) * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0))); // rotate the quad to show normal mapping from multiple directions
